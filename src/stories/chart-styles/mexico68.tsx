...
export const MEXICO_PALETTE = [MEXICO.pink, MEXICO.orange, MEXICO.purple, MEXICO.cyan, MEXICO.green] as const;
...
const toArchSegment = (observation: Observation, mainAxis: MainAxis, key: string): ArchSegment | null => {
  ...
  return {
    ...
    color: getColor(observation) ?? MEXICO.ink,
    opacity: getAlpha(observation) ?? 1,
    cap: getStackCap(observation),
  };
};
...
const toLineTrace = (observations: Observation[], key: string): LineTrace | null => {
  ...
  return {
    key,
    points,
    color: getColor(first) ?? MEXICO.ink,
