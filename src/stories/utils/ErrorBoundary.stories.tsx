...
const WARNING_MODES: FailureMode[] = [
  ...
  },
  {
    ...
    code: 'INVALID_HIGHLIGHT_OPERATOR',
    spec: pipe(
      createSpec(),
      mapping({ x: 'category', y: 'revenue' }),
      geom.bar(),
  ...
  {
    label: 'Highlight targets a layer index out of range (drops the highlight)',
    ...
    spec: pipe(
      ...
      scale.x(),
      scale.y(),
      highlight({ variable: 'category', eq: 'Product A' }, { layerIndex: 5 })
    ),
  },
  {
    label: 'Tinted background with no resolvable colour (falls back to the theme background)',
    ...
    spec: pipe(
      ...
      scale.x(),
      scale.y(),
      config({ appearance: { background: { type: 'tinted' } } })
    ),
  },
  {
